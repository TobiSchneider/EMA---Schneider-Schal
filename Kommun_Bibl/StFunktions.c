...
void anySTProz( void ) {
    ...
    // Senden Flag 0
    //uint8_t stateVal = 0x0000 ;
    /*Hier sind unterschiedliche Gründe der Termination beschrieben*/
    ...
    if( 1/*Wenn timeout ist out*/ ) {   // time out process by the charger, change #109.5.4 (Charger system malfunction)
        /*Übergang in Termination*/
        // Notiz: Zunächst soll der Time out detektiert werden un der 1 Bit in CAN gesendet werden
...
void stateAtcionFct(int state) {
    switch(state) {
        case ST_ANY :
            anySTProz() ;   // Hier werden alle Fälle, die zum Termination führen bescrhrieben
        break ;

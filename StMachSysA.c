...
tTransition trans[] = {
                        ...
                        { ST_START_CHARGIN_OUTPUT, EV_CHECK_PASSED , 10, &noChange },
                        { ST_START_CHARGIN_OUTPUT, EV_START_CHARGING_I_REQ , 10, &noChange },    // Bleibt in Ladezustand , so lange das Fahrzeug die Current Requests sendet
                        ...
                        { ST_ANY,                  EV_CHECK_PASSED , 10, &noChange },  // Falls kein EV einkommt, bleibt das System in altem Zustand
                        /******************************************************************************************/


                        /**************************DER KERN DER CHADEMO ZUSTANDSMASCHINE**************************/
                        { ST_INIT,   EV_INITIALISAT_COMPLETE, 10, &chargeStartButt },                       // Termination der Initialisation.

                        { ST_CHARGE_START__BUTTON, EV_CHARGE_START_BUTT_ON, 10, &transChargeStartSig   },   // | |Send start-of-charging signal |
                        { ST_TRANS_CHARGE_STR_SIG, EV_TRA_CHAR_STR_SIG_COM, 10, &recTraInitCANData     },   //  → Charger pulls Pin 2 up to 12v |
                        { ST_REC_TRA_INI_CAN_DATA, EV_START_CAN_COMMUNICAT, 10, &procInfoForChargeCont },   // Recognize start-of-charging
                        { ST_PROC_INF_CHARGE_CONT, EV_PROC_INF_CHARGE_CONT, 10, &lockChargingConnec    },   // | | Recieve battery parameters, do a compatibility check | ← Send over CAN link | Transmit battery parameters + | |Transmit charger parameters | → Send over CAN Bus | Receive charger parameters, do compatibility check
                        { ST_LOCK_CHARGING_CONNEC, EV_TRAN_SIG_VIH_SET_COM, 10, &checkEVContOpened     },   // | |Recognize start Permission Signal | ← Car grounds pin 4 | Send Start Permission Signal
                        { ST_CHECK_EV_CONT_OPENED, EV_EV_CONT_IS_OPEN_COMP, 10, &insulTestOnOutCirc    },   // | | Connector lock(? oben ?) and
                        { ST_INSU_TEST_OUT_DC_CIR, EV_INSU_OUT_DC_CIR_COMP, 10, &checkTeminInsulTest   },   // perform insulation test,
                        { ST_CHECK_TERM_INSU_TEST, EV_V_OUT_TERM_LESS_20_V, 10, &transSigOfChargiComp  },   //
                        { ST_TRANS_SIG_CHARGE_COM, EV_TRA_SIG_CHAR_COM_COM, 10, &startChargingOut      },   // send charging ready signal( switch(d2):ON ) |→ Charger grounds pin 10 | Recognize charging ready signal, main contactor ON
                        //{ ST_START_CHARGIN_OUTPUT, EV_START_CHARGING_I_REQ, &noChange },
                        { ST_START_CHARGIN_OUTPUT, EV_TRAN_CHARGIN_STOP_RQ, 10, &stopCharging          },   // | |Receives current requirement, adjust to within 2% output | ← Send current requirement every 100ms over CAN Bus | BMS monitors battery SOC, Temp, and input current, calculates current requirements
                        { ST_STOP________CHARGING, EV_STOP____CHARGING_COM, 10, &checkDCVoltLess10V    },   // | | Charging reaches end SOC set in charger | ← OR → | Stopped by user
                        ...
                        { ST_UNLOCK_CHARGING_CONN, EV_UNLOCK_CHAR_CONN_COM , 10, &termCANCommun },          // | |Terminate Charging Process | → Charger disconnects Pins 2 & 10, unlocks connector | User Removes CHAdeMO connector from Vehicle |
                        /*******************************************************************************************/
...
/*->*/                       { ST_PROC_INF_CHARGE_CONT, EV_CHRG_VOLT_REACH_THRS, 10, &stopCharging },
                        ...
                        { ST_ANY, EV_ANY, 10, &error }  // bei jedem ST und jedem EV werden die Fehlerzustand geprüft. Hier landet das System falls undefinierter ST, oder EV vorkommt.
} ;
